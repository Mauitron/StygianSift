            ...
            if !cfg!(target_os = "windows") || key.kind == KeyEventKind::Press {
                if let Some(action) = app_state.config.get_action(&key) {
                    ...
                    match action {
                        ...
                        Action::ToggleCount => {
                            app_state.show_count = !app_state.show_count; // not in use as of yet.
                        }
                        ...
                        Action::Murder => {
                            if let Some(_entry) = entries.get(selected_index as usize) {
                                murder_files(app_state, &mut stdout, &entries, selected_index)?;
                            }
                        }
                        Action::Copy => {
                        ...
                        Action::Duplicate => {
                            handle_duplicate(app_state, &mut stdout, &entries, selected_index)?;
                        }
                        Action::MoveItem => {
                            handle_move_item(
                                app_state,
